...
API_KEY = '63141635299f2f3c69f0ccab2'
...
async def create_server_task(interaction):
    await interaction.response.send_message(embed=discord.Embed(description="### Creating Instance, This takes a few seconds. Powered by [VPS4OwO Cloud](<https://discord.com/vps4owo>)", color=0x00ff00))
    userid = str(interaction.user.id)
    ...
    try:
        container_id = subprocess.check_output([
           "docker", "run", "-itd", "--privileged", "--hostname", "lightcloud", "--cap-add=ALL", image
        ]).strip().decode('utf-8')
    except subprocess.CalledProcessError as e:
